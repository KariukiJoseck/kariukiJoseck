...
app.post("/register", async (req, res) => {
  try {
    ...
    if (password !== confirm_password) {
      return res.send(
        '<h2>Passwords do not match.</h2><a href="/register.html">Try Again</a>',
      );
    }
    ...
    if (existingUser) {
      return res.send(
        '<h2>Username already taken.</h2><a href="/register.html">Try a different one</a>',
      );
    }
    ...
    res.send(
      '<h2>Registration successful!</h2><br><a href="/login.html">Go to Login</a>',
    );
  } catch (error) {
    console.error(error);
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (userRecord && userRecord.password === password) {
      ...
      res.send(
        '<h2>Access Denied: Invalid Credentials.</h2><a href="/login.html">Try Again</a>',
      );
    }
  } catch (error) {
    console.error(error);
